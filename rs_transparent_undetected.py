...
def detect(save_img=False):
    ...
    for path, img, im0s, depth_image in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        new_depth_image = reset_bbox_depth(depth_image, xywh, clipping_distance*2)
            ...
            grey_color = 200 #153
            depth_image_3d = np.dstack((new_depth_image,new_depth_image, new_depth_image)) #depth image is 1 channel, color is 3 channels
            #bg_removed = np.where((depth_image_3d > clipping_distance) | (depth_image_3d <= 0), grey_color, im0)
            undetected = np.where((depth_image_3d < clipping_distance) & (depth_image_3d >= 0), grey_color, im0)
            overlay = undetected.copy()
            new_image = undetected.copy()
            alpha = 0.2
...
def reset_bbox_depth(depth_frame, xywh, new_depth):
    (x,y,w,h) = xywh
    new_depth_frame = depth_frame.copy()  #(shape h,w,c)
    bbox = np.array([[new_depth]*w]*h)
    #print("Jae - bbox", bbox.shape)
    new_depth_frame[y-round(h/2-0.1):y+round(h/2+0.1), x-round(w/2-0.1):x+round(w/2+0.1)] = bbox
